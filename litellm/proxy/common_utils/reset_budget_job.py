...
class ResetBudgetJob:
    ...
    async def reset_budget_for_litellm_team_members(self, budgets_to_reset: List[LiteLLM_BudgetTableFull]):
        ...
        """
        budget_ids = [budget.budget_id for budget in budgets_to_reset if budget.budget_id is not None]
        ...
        try:
            ...
            memberships = []
            verbose_proxy_logger.warning("Failed to fetch team memberships for counter invalidation: %s", e)
        ...
        for m in memberships:
            await self._invalidate_spend_counter(f"spend:team_member:{m.user_id}:{m.team_id}")
    ...
    async def reset_budget_for_keys_linked_to_budgets(self, budgets_to_reset: List[LiteLLM_BudgetTableFull]):
        ...
        try:
            keys = await self.prisma_client.db.litellm_verificationtoken.find_many(where=where_clause)
        except Exception as e:
            keys = []
            verbose_proxy_logger.warning("Failed to fetch keys for counter invalidation: %s", e)

        update_result = await self.prisma_client.db.litellm_verificationtoken.update_many(
            where=where_clause,
            data={
                "spend": 0,
            },
        ...
        try:
            ...
            if budgets_to_reset is not None and len(budgets_to_reset) > 0:
                ...
                if litellm.max_end_user_budget_id is not None and litellm.max_end_user_budget_id in budget_ids_to_reset:
                    default_budget_endusers = await self._get_endusers_with_no_budget_id()
    ...
    async def reset_budget_windows(self) -> None:
        ...
        # --- End-Users ---
        try:
            end_user_rows = await self.prisma_client.db.query_raw(
                'SELECT user_id, budget_limits FROM "LiteLLM_EndUserTable" WHERE budget_limits IS NOT NULL'
            )
            for row in end_user_rows:
                ...
                changed = False
                for window in windows:
                    counter_key = f"spend:end_user:{row['user_id']}:window:{window['budget_duration']}"
                    if await ResetBudgetJob._reset_expired_window(window, counter_key, spend_counter_cache, now):
                        changed = True
                if changed:
                    await self.prisma_client.db.litellm_endusertable.update(
                        where={"user_id": row["user_id"]},
                        data={"budget_limits": json.dumps(windows)},  # type: ignore[arg-type]
                    )
        except Exception as e:
            verbose_proxy_logger.exception("Failed to reset budget windows for end users: %s", e)
    ...
    async def _reset_budget_common(
        ...
        try:
            ...
            if item_type == "key" and hasattr(item, "token") and item.token is not None:  # type: ignore[union-attr]
                ...
                item_type == "team" and hasattr(item, "team_id") and item.team_id is not None  # type: ignore[union-attr]
            ):
